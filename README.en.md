...
### Recommended: give the link to Codex

After the project is published on GitHub, copy the URL of the repository's `codex-grok-search/` directory, start a new Codex task, and send:

```text
Please install this Skill: <URL of the codex-grok-search directory on GitHub>
```
...
> The public GitHub repository does not exist yet, so this draft intentionally does not invent an installation URL. The placeholder above will be replaced with the real, copyable link before publication.
...
This repository has not yet been published on GitHub, so this draft does not invent a Release URL. The version number and Release page will be locked together before publication. Restart Codex or start a new task after installation.
