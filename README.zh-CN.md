...
> 当前稳定版本：`v0.1.1`。项目非官方，与 xAI、X、Reddit 或 OpenAI 均无隶属关系。
...
从 GitHub Release 页面下载下列两个必需的 `v0.1.1` 资产，并放在同一个目录：

- `codex-grok-search-v0.1.1.zip`
- `SHA256SUMS`

`codex-grok-search-v0.1.1.tar.gz` 是可选的内容相同备用格式。下面的命令只校验并安装你选择的 ZIP，不要求同时下载 tar.gz；随后验证 Skill 结构并完整替换旧版本。任何校验或切换失败都会保留或恢复原安装，不会把新旧文件合并。
...
请从 [v0.1.1 Release 页面](https://github.com/sudoHG/codex-grok-search/releases/tag/v0.1.1)下载资产。安装完成后，重启 Codex 或新建一个任务。
