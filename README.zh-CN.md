...
Codex 负责理解任务和整理答案，Grok 负责发现 X、Reddit 与网页上的公开内容。本地包装器只承担两个产品职责：让 Grok 在当前仓库之外运行，并完整保存 Grok 的回答；它不筛选或拒绝调研内容。

> 当前稳定版本：`v0.1.4`。项目非官方，与 xAI、X、Reddit 或 OpenAI 均无隶属关系。
...
- 即使 Grok 混合了不同平台来源、使用自由 Markdown、包含 `http` 链接或无法确认日期，也照常返回完整回答。
...
`codex-grok-search` 不接管你的 X 或 Reddit 登录，不读取浏览器 Cookie，不在本机运行平台页面自动化，也不在本地二次抓取 Reddit 页面。X、Reddit 和网页发现都交给 xAI 的服务端工具执行。这可以减少由用户机器直接发起的抓取量，以及账号、Cookie 和本机 IP 暴露给自动化风控的机会。

但它不是“防封保证”：平台政策、Grok 可用性、公开内容可见范围和服务端限额仍可能变化。
...
`codex-grok-search` 不在用户当前项目或 Git 仓库中启动 Grok。它在仓库之外创建仅当前用户可访问的独立研究目录，只写入本次搜索 prompt 和结果文件，再把该目录设为 Grok 的 `--cwd`。真实代码仓库不会被复制、挂载或传入这个运行环境。Grok 同时使用临时 `HOME`、`GROK_HOME` 和 `TMPDIR`，其中只包含认证文件副本以及禁止导入兼容配置的最小配置。

这个保护不依赖 Grok 的“不上传仓库”承诺：即使 CLI 再次尝试打包整个工作目录，它面对的也只是本次临时研究目录，而不是用户的代码仓库。

正式研究只保留少量固定边界：

- 使用本机正常的 Grok 可执行文件，直接发起真实搜索；不再设置单独的版本、登录、模型或 `inspect` 前置门禁。
- 每个任务都开放 `x_search`、`web_search` 和 `web_fetch`；平台选项只是提示，不是排他规则。
- 不向模型开放 MCP、本地文件读取、Shell、文件编辑、记忆或子代理。
- Grok 回答原样保存，不再做平台、URL、字段、日期或 Markdown 校验。
- Grok 已经返回可用内容后即使进程随后非零退出，也会标为 `partial` 并交付，而不是整包丢弃。
...
- Python 3.9 或更高版本。
- 通过 [xAI 官方安装器](https://x.ai/cli) 安装的 Grok Build CLI。Skill 不锁定或拒绝 CLI 版本。
...
- 当前账号无法使用 Grok 4.5：由真实 Grok 调用直接返回错误。
...
从 GitHub Release 页面下载下列两个必需的 `v0.1.4` 资产，并放在同一个目录：

- `codex-grok-search-v0.1.4.zip`
- `SHA256SUMS`

`codex-grok-search-v0.1.4.tar.gz` 是可选的内容相同备用格式。下面的命令只校验并安装你选择的 ZIP，不要求同时下载 tar.gz；随后验证 Skill 结构并完整替换旧版本。任何校验或切换失败都会保留或恢复原安装，不会把新旧文件合并。
...
请从 [v0.1.4 Release 页面](https://github.com/sudoHG/codex-grok-search/releases/tag/v0.1.4)下载资产。安装完成后，重启 Codex 或新建一个任务。
...
- 包装器原样保存 Grok 的回答，不再在本地二次验证 Reddit 日期。
- 平台提示不会排除其他平台上的有用来源。
- `http` 链接、自由 Markdown、无法验证的日期、字段缺失和多来源混合都不会导致结果被拒绝。
- 搜索覆盖与准确性仍是尽力而为；Codex 可以说明不确定性，但不会静默删除 Grok 已返回的结果。
