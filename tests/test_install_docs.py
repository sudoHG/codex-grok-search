...
RELEASE_URL = (
    "https://github.com/sudoHG/codex-grok-search/releases/tag/v0.1.4"
)
BADGE_LABELS = (
    "CI",
    "Release",
...
class InstallDocumentationTests(unittest.TestCase):
    def test_skill_metadata_explicitly_routes_current_x_and_reddit_first(self):
        ...
        self.assertIn("latest posts", frontmatter)
        self.assertIn("recent discussions", frontmatter)
        self.assertIn("without content filtering", frontmatter)
        self.assertIn("Do not open returned links", skill)
        self.assertIn("Do not reject, delete, rewrite, or filter", skill)
    ...
    def prepare_release(self, root: Path, validator_exit: int = 0):
        ...
        archive = assets / "codex-grok-search-v0.1.4.zip"
        assets.mkdir()
        ...
        tarball = assets / "codex-grok-search-v0.1.4.tar.gz"
    ...
    def test_release_install_verifies_and_replaces_without_stale_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (assets / "codex-grok-search-v0.1.4.tar.gz").unlink()
            destination = codex_home / "skills" / "codex-grok-search"
            destination.mkdir(parents=True)
            (destination / "stale.txt").write_text("old\n", encoding="utf-8")
